...
import pojos.Login;
import pojos.Mapper;
import pojos.Mensagem;
import pojos.Token;
import pojos.postUser;
import pojos.usersGroup;
...
//	}
	...
	public Response login(final Login login) throws JSONException {
		...
		if(StringUtils.equals(token, "usuário e senha inválido")){
			return Response.status(400).entity(new Mensagem("usuário e senha inválido")).build();
		}
		
		return Response.status(200).entity(new Token(token)).build();
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.TEXT_HTML)
	public Response postUsersGroup(final usersGroup group) throws JSONException {
		RMIServerAPI rmi = getServerRMI();
		...
		try {
			...
			String postUser = rmi.setUsersGroup(group.name, users, group.token);
